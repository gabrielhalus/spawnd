...
// Exclusions on remote during cleaning
const exclude = ["uploads", "logs", ".env", "data"];

// Local files to push for server (backend)
const localServerFiles = [
  "apps/backend", // Include entire backend app
  "packages/shared", // Include shared package
  "bun.lock", // Lock file
  "package.json", // For workspaces
  "tsconfig.json", // Shared config if used
  "Dockerfile", // Root Dockerfile
  "docker-compose.yml", // Compose file
];

// Only built frontend (no source)
const localClientFiles = [
  "apps/frontend/dist", // Final built app
  ...
  try {
    switch (subcommand) {
      ...
      default:
        ...
        console.log("\nUsage:");
        console.log(
          "  bun run scripts/deploy.ts [deploy|clean|push|restart|down]",
        );
        process.exit(1);
    }
  } catch (err) {
